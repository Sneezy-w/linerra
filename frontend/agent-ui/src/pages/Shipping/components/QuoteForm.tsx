...
import { Steps, Row, Col, Card, Form, Select, Input, InputNumber, Button, Space, Typography, Spin } from 'antd';
import ProTable from '@ant-design/pro-table';
import { FooterToolbar, ProCard, ProFormDigitRange, ProFormField, ProFormFieldSet, ProFormGroup, ProFormInstance, ProFormList, ProFormListProps, ProFormTextArea } from '@ant-design/pro-components';
...
const QuoteForm: React.FC = () => {
  ...
  const packagesProFormListRender = (params: Record<string, any>) => (meta: any, index: any, action: any, count: any) => {
    const weightInput = <ProFormDigit
      name='weight'
      label='Weight'
    ...
    const dimensionAndInsuranceInput = params?.package?.type === 'parcel' && (
      <>
        <ProForm.Item label='Dimensions' name='dimension' initialValue={{ length: 1.00, width: 1.00, height: 1.00 }} rules={[{
          validator: (_, value) => {
        ...
        <ProFormDigit name='insurance' label='Carrier Insurance' min={0} placeholder='Optional.' initialValue={0.00}
          fieldProps={{
            addonAfter: '$',
            precision: 2,
          }}
        ...
        >
          <ProCard ghost gutter={[16, 16]} direction='row' type='inner'>
            ...
            <ProCard colSpan={12} ghost direction='column' gutter={[0, 16]} loading={loading}>
              <ProCard bordered title="SHIP FROM" headerBordered type='inner' loading={loading} extra={
              ...
              <ProCard bordered title="Options" headerBordered type='inner' loading={loading}>
                <ProFormDigit name={['option', 'packingFee']} label="Packing Fee" min={0} placeholder="Optional." initialValue={0.00}
                  fieldProps={{
                    addonBefore: '$',
                    precision: 2,
                  }} />
                <ProFormText name={['option', 'memo']} label="Remark" placeholder="Optional." extra={
                  <Text type="secondary" >
                    <QuestionCircleOutlined style={{ marginRight: 4 }} />
            ...
            <ProCard colSpan={12} ghost direction='column' gutter={[0, 16]} loading={loading}>
              <ProCard bordered title="SHIP TO" headerBordered type='inner' loading={loading} extra={
                <Button type="primary" icon={<ContactsOutlined />} />
              }>
                <ProFormSelect
                  fieldProps={{
